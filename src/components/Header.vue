<template>
  <div class="flex flex-col items-center lg:items-end lg:mb-auto max-w-sm mx-auto">
    <div class="bg-slate-600 h-20 w-20 mb-6 rounded-full">
      <img class="h-full w-full rounded-full" href="#"/>
    </div>
    <h1 class="sm:max-lg:bg-black/60 rounded sm:max-lg:px-8 text-center text-2xl lg:text-xl text-gray-200"><strong>I am Keith Estanislao.</strong> Developer, gamer, and father</h1>
  </div>
</template>

<script setup>

</script>
